...
public class TravelDAOImplement implements TravelDAO {
    ...
    @Override
    public void insertData(Object info) {
        try (Connection conn = getConnection()) {
            if (info instanceof ContinentVO) {
                ContinentVO vo = (ContinentVO) info;
                String sql = "INSERT INTO Continent (continent_id, continent_name) VALUES (?, ?)";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, vo.getId());
                    pstmt.setString(2, vo.getName());
                    pstmt.executeUpdate();
                }
            } else if (info instanceof CountryVO) {
                CountryVO vo = (CountryVO) info;
                String sql = "INSERT INTO Country (country_id, country_name, continent_id) VALUES (?, ?, ?)";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, vo.getId());
                    pstmt.setString(2, vo.getName());
                    pstmt.setString(3, vo.getContinentId());
                    pstmt.executeUpdate();
                }
            } else if (info instanceof RestaurantVO) {
                RestaurantVO vo = (RestaurantVO) info;
                String sql = "INSERT INTO Restaurant (restaurant_id, restaurant_name, country_id, address, menu) VALUES (?, ?, ?, ?, ?)";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, vo.getId());
                    pstmt.setString(2, vo.getName());
                    pstmt.setString(3, vo.getCountryId());
                    pstmt.setString(4, vo.getLocation());
                    pstmt.setString(5, vo.getRecommendMenu());
                    pstmt.executeUpdate();
                }
            } else if (info instanceof TouristVO) {
                TouristVO vo = (TouristVO) info;
                // TouristVo 객체 생성해서 vo라고 명명 이후, info를 자료형 변환함
                String sql = "INSERT INTO Tourist (tourist_id, tourist_name, country_id, location) VALUES (?, ?, ?, ?)";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, vo.getId());
                    pstmt.setString(2, vo.getName());
                    pstmt.setString(3, vo.getCountryId());
            ...
            } else if (info instanceof TravelTipVO) {
                TravelTipVO vo = (TravelTipVO) info;
                String sql = "INSERT INTO TravelTip (tip_id, country_id, entry_info, flight_info, time_diff, payment_info, transport, food_info, stay_info, shopping, weather, safety, etc) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setString(1, vo.getId());
                    pstmt.setString(2, vo.getCountryId());
                    pstmt.setString(3, vo.getEntryInfo());
                    pstmt.setString(4, vo.getFlightInfo());
                    pstmt.setString(5, vo.getTimeDiff());
                    pstmt.setString(6, vo.getPaymentInfo());
                    pstmt.setString(7, vo.getTransport());
                    pstmt.setString(8, vo.getFoodInfo());
                    pstmt.setString(9, vo.getStayInfo());
                    pstmt.setString(10, vo.getShopping());
                    pstmt.setString(11, vo.getWeather());
                    pstmt.setString(12, vo.getSafety());
                    pstmt.setString(13, vo.getEtc());
                    pstmt.executeUpdate();
                }
            } else {
                System.out.println("지원하지 않는 데이터 타입: " + info.getClass().getName());
            }
        } catch (SQLException e) {
    ...
    public void deleteData(String id) {
        System.out.println("삭제 기능은 미구현 상태입니다. 삭제할 ID: " + id);
    }
}
